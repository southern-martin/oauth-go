...
func getAccessToken(accessTokenId string) (*accessToken, rest_error.RestErr) {
	...
	if response.StatusCode > 299 {
		restErr, err := rest_error.NewRestErrorFromBytes(response.Bytes())
		if err != nil {
			return nil, rest_error.NewInternalServerError("invalid error interface when trying to get access token", err)
		}
		return nil, restErr
	}
